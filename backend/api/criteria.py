...
from models.db_models import SystemPrompt, EvaluationCriteria
...
@router.get("/{scenario}")
def get_criteria(scenario: str, db: Session = Depends(get_db)):
    """
    Retrieves the currently saved criteria for a specific scenario (e.g., 'MS2').
    """
    criteria_record = db.query(EvaluationCriteria).filter(EvaluationCriteria.scenario_name == scenario).first()
...
@router.put("/save")
def save_criteria(request: SaveCriteriaRequest, db: Session = Depends(get_db)):
    """
    Saves the final generated Markdown criteria to the database for future evaluation use.
    """
    criteria_record = db.query(EvaluationCriteria).filter(EvaluationCriteria.scenario_name == request.scenario).first()
    ...
    db.commit()
    return {"message": f"Kritéria pro {request.scenario} úspěšně uložena.", "content": request.markdown_content}
