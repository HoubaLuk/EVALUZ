...
@router.put("/prompts")
def update_prompts(updates: List[PromptUpdateInfo], db: Session = Depends(get_db)):
...
@router.put("/settings")
def update_settings(updates: List[AppSettingUpdateInfo], db: Session = Depends(get_db)):
...
async def test_connection(config: TestConfigRequest):
    """
