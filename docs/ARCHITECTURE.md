...
- **StudyGroup / Scenario [od v3.17.0, ADR-033]:** Strom vlevo. `Scenario.scenario_key` odpovídá `scenario_name` v kritériích i vyhodnoceních, ale **generuje ho server** — dřív vznikal na klientovi jako `scen-${Date.now()}` a situace existovala jen jako položka stromu v prohlížeči, takže šla ztratit i s cestou k datům. Unikátní je **v rámci lektora**, ne globálně (historické klíče `scen-1`/`scen-2` má z výchozí šablony každý). `StudyGroup` NENÍ `ClassRoom`: ta je analytický kbelík pro `class_id`, tohle je složka ve stromu.
